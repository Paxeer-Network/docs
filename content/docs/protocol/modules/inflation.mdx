...
The `x/inflation` module mints new HyperPaxeer tokens and allocates them in daily
epochs according to the [HyperPaxeer Token
Model](https://Paxeer-Network.blog/the-HyperPaxeer-token-model-edc07014978b) distribution to
...
The allocation of new coins incentivizes specific behaviour in the HyperPaxeer network.
...
### HyperPaxeer Token Model

The HyperPaxeer Token Model outlines how the HyperPaxeer network is secured through a
...
issue 1 billion HyperPaxeer tokens within the first 4 years.
...
a daily provision (`epochProvison`) of HyperPaxeer tokens is minted
...
Note, that after [discussion](https://www.mintscan.io/Paxeer-Network/proposals/258)
with the validator community, it was decided to decrease the inflation to 1/3
during the upgrade to [v16.0.0](https://www.mintscan.io/Paxeer-Network/proposals/265).
...
Find below a list of `hyperpaxd` commands added with the `x/inflation` module. You
...
```bash
hyperpaxd query inflation period [flags]
```
...
```bash
hyperpaxd query inflation epoch-mint-provision [flags]
```
...
```bash
hyperpaxd query inflation skipped-epochs [flags]
```
...
```bash
hyperpaxd query inflation total-supply [flags]
```
...
```bash
hyperpaxd query inflation inflation-rate [flags]
```
...
```bash
hyperpaxd query inflation params [flags]
```
...
| ------ | --------------------------------------------- | --------------------------------------------- |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/Period`             | Gets current inflation period                 |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/EpochMintProvision` | Gets current inflation epoch provisions value |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/Params`             | Gets current inflation parameters             |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/SkippedEpochs`      | Gets current number of skipped epochs         |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/TotalSupply`        | Gets current total supply                     |
| `gRPC` | `HyperPaxeer.inflation.v1.Query/InflationRate`      | Gets current inflation rate                   |
