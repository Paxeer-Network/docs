...
Make sure to look at [HyperPaxeer Governance](https://academy.evmosd.org/articles/governance/),
and specifically the [best practices](https://academy.evmosd.org/articles/governance/best-practices).
:::

## Abstract

This document specifies the internal `x/erc20` module of the HyperPaxeer Hub.

The `x/erc20` module enables the HyperPaxeer Hub to support a trustless, on-chain bidirectional internal conversion of tokens
between HyperPaxeer' EVM and Cosmos runtimes, specifically the `x/evm` and `x/bank` modules.
This allows token holders on HyperPaxeer to instantaneously convert their native Cosmos `sdk.Coins`
...
This conversion functionality is fully governed by native HyperPaxeer token holders
...
with the difference, that it not only applies to gas tokens (like HyperPaxeer),
but to all Cosmos Coins (IBC vouchers, staking and gov coins, etc.) as well.

With the `x/erc20` users on HyperPaxeer can
...
- transfer existing tokens on Ethereum and other EVM-based chains to HyperPaxeer
...
It can be either the native staking/gas denomination (e.g. HyperPaxeer, ATOM, etc)
...
Holders of native Cosmos coins and IBC vouchers on the HyperPaxeer chain can convert their Coin into ERC20 Tokens,
which can then be used in HyperPaxeer EVM, by creating a `ConvertCoin` Tx.
Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the HyperPaxeer chain
...
please review the x/erc20 audit, section `IF-HyperPaxeer-06: IERC20 Contracts may execute arbitrary code`.
...
type TokenPair struct {
	...
	ContractOwner Owner `protobuf:"varint,4,opt,name=contract_owner,json=contractOwner,proto3,enum=HyperPaxeer.erc20.v1.Owner" json:"contract_owner,omitempty"`
}
```
...
Note that the native HyperPaxeer coin cannot be registered,
as any coin including "evm" in its denomination cannot be registered.
Instead, the HyperPaxeer token can be converted by Nomand's wrapped HyperPaxeer (WHyperPaxeer) contract.

1. User submits a `RegisterCoinProposal`
2. Validators of the HyperPaxeer Hub vote on the proposal using `MsgVote` and proposal passes
3. If Cosmos coin or IBC voucher exist on the bank module supply,
   create the [ERC20 token contract](https://github.com/Paxeer-Network/Paxeer-Network/blob/main/contracts/ERC20MinterBurnerDecimals.sol)
...
2. Validators of the HyperPaxeer chain vote on the proposal using `MsgVote` and proposal passes
...
- There shouldn't exist any native Cosmos Coin ERC20 Contract (eg HyperPaxeer, Atom,
...
Find below a list of  `hyperpaxd` commands added with the  `x/erc20` module.
You can obtain the full list by using the `hyperpaxd -h` command.
A CLI command can look like this:

```bash
hyperpaxd query erc20 params
```
...
```bash
hyperpaxd tx gov submit-legacy-proposal register-coin METADATA_FILE [flags]
```
...
```bash
hyperpaxd tx gov submit-legacy-proposal register-erc20 ERC20_ADDRESS... [flags]
```
...
```bash
hyperpaxd tx gov submit-legacy-proposal toggle-token-conversion TOKEN [flags]
```
...
For more information on how to draft a proposal, refer to the [Drafting a proposal section](../Paxeer-Network-cli/proposal-draft.md).

```bash
hyperpaxd tx gov submit-proposal proposal.json [flags]
```
...
| ------ | --------------------------------- | ------------------------------ |
| `gRPC` | `HyperPaxeer.erc20.v1.Query/Params`     | Get erc20 params               |
| `gRPC` | `HyperPaxeer.erc20.v1.Query/TokenPair`  | Get registered token pair      |
| `gRPC` | `HyperPaxeer.erc20.v1.Query/TokenPairs` | Get all registered token pairs |
| `GET`  | `/Paxeer-Network/erc20/v1/params`          | Get erc20 params               |
| `GET`  | `/Paxeer-Network/erc20/v1/token_pair`      | Get registered token pair      |
| `GET`  | `/Paxeer-Network/erc20/v1/token_pairs`     | Get all registered token pairs |

#### Transactions

| Verb   | Method                             | Description                    |
| ------ | ---------------------------------- | ------------------------------ |
| `gRPC` | `HyperPaxeer.erc20.v1.Msg/ConvertCoin`   | Convert a Cosmos Coin to ERC20 |
| `gRPC` | `HyperPaxeer.erc20.v1.Msg/ConvertERC20`  | Convert a ERC20 to Cosmos Coin |
| `GET`  | `/Paxeer-Network/erc20/v1/tx/convert_coin`  | Convert a Cosmos Coin to ERC20 |
| `GET`  | `/Paxeer-Network/erc20/v1/tx/convert_erc20` | Convert a ERC20 to Cosmos Coin |
