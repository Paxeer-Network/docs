...
HyperPaxeer can dump the entire application state to a JSON file.
...
```bash
hyperpaxd export > new_genesis.json
```
...
```bash
hyperpaxd export --height [height] > new_genesis.json
```
...
```bash
hyperpaxd export --height [height] --for-zero-height > new_genesis.json
```
...
```bash
hyperpaxd migrate TARGET_VERSION GENESIS_FILE --chain-id=<new_chain_id> --genesis-time=<yyyy-mm-ddThh:mm:ssZ>
```
