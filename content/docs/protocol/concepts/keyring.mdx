...
```bash
hyperpaxd keys add dev0

# Put the generated address in a variable for later use.
MY_VALIDATOR_ADDRESS=$(hyperpaxd keys show dev0 -a)
```
...
:::warning
**NOTE**: Cosmos `secp256k1` keys are not supported on HyperPaxeer due to compatibility issues with Ethereum transactions.
:::
...
yes $KEYPASSWD | hyperpaxd keys add me
yes $KEYPASSWD | hyperpaxd keys show me
# start hyperpaxd with keyring-backend flag
hyperpaxd --keyring-backend=file start
```
