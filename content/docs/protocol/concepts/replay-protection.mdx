...
However, as HyperPaxeer also accepts EVM transactions, handling only those transactions that conform to EIP-155
...
By default, replay protection is enabled on any HyperPaxeer node.
...
by setting the `allow_unprotected_txs` field to `true` in the genesis of a [local node](../Paxeer-Network-cli/single-node.mdx).
