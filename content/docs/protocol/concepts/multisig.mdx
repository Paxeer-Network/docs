...
A **multisig account** is an HyperPaxeer account with a special key that can require more than one signature to sign transactions.
...
```bash
hyperpaxd keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
hyperpaxd keys add --multisig=p1,p2,p3 --multisig-threshold=2 multisig_address
hyperpaxd keys add --multisig=p2,p3,p1 --multisig-threshold=2 multisig_address
```

Multisig addresses can also be generated on-the-fly and printed through the which command:

```bash
hyperpaxd keys show --multisig-threshold=K name1 name2 name3 [...]
```
...
hyperpaxd keys add \
    test3 \
    --pubkey=HyperPaxeerpub1addwnpepqgcxazmq6wgt2j4rdfumsfwla0zfk8e5sws3p3zg5dkm9007hmfysxas0u2
```
...
```sh
hyperpaxd keys show multi

- name: multi
  type: multi
  address: HyperPaxeer1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m
...
Let's add 10 HyperPaxeer to the multisig wallet:

```bash
hyperpaxd tx bank send \
    test1 \
    HyperPaxeer1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m \
    10000000000000000000ahpx \
    --chain-id=hyperpax_125-4 \
    --gas=auto \
    --fees=1000000ahpx \
...
We want to send 5 HyperPaxeer from our multisig account to `HyperPaxeer1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft`.

```bash
hyperpaxd tx bank send \
    HyperPaxeer1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft \
    HyperPaxeer157g6rn6t6k5rl0dl57zha2wx72t633axqyvvwq \
    5000000000000000000ahpx \
    --gas=200000 \
    --fees=1000000ahpx \
    --chain-id=hyperpax_125-4 \
    ...
    "messages": [
      {
        ...
        "from_address": "HyperPaxeer1rgjxswhuxhcrhmyxlval0qa70vxwvqn2e0srft",
        "to_address": "HyperPaxeer157g6rn6t6k5rl0dl57zha2wx72t633axqyvvwq",
        "amount": [
          {
            "denom": "ahpx",
...
hyperpaxd tx sign \
    unsignedTx.json \
    --multisig=HyperPaxeer1e0fx0q9meawrcq7fmma9x60gk35lpr4xk3884m \
    --from=test1 \
...
```sh
hyperpaxd tx broadcast signedTx.json \
    --chain-id=hyperpax_125-4 \
    --broadcast-mode=block
```
